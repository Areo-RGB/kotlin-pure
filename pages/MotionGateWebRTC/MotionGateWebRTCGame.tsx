...
import { db, getServerTime } from "../../services/firebase";
import {
  ref,
  onChildAdded,
  set,
...
const MotionGateWebRTCGame: React.FC = () => {
  ...
  const [deviceName] = useState(
    `Device ${deviceId.substr(0, 3).toUpperCase()}`
  );
  ...
  const [history, setHistory] = useState<
    {
      id: number;
      duration: number;
      splits: { duration: number; deviceName: string }[];
    }[]
  >([]);
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      (Object.values(peerConnections.current) as RTCPeerConnection[]).forEach(
        (pc) => pc.close()
      );
    };
  }, [lobbyId, wssUrl]);
  ...
  const sendSignal = (targetId: string, payload: any) => {
    if (isWssMode) {
      ...
      const targetSignalRef = ref(
        db,
        `lobbies/${lobbyId}/webrtc/signals/${targetId}`
      );
  ...
  const createPC = (peerId: string, isManual = false) => {
    ...
    const iceServers =
      isManual || isWssMode ? [] : [{ urls: "stun:stun.l.google.com:19302" }];
    ...
    pc.onconnectionstatechange = () => {
      ...
      updatePeerUI(
        peerId,
        pc.connectionState,
        dataChannels.current[peerId]?.readyState || "closed",
        isManual
      );
      if (
  ...
  const handleSignal = async (
    ...
    if (!pc) {
      if (data.type === "offer") {
        ...
        pc.ondatachannel = (e) =>
          setupDataChannel(senderId, e.channel, isManual);
        pc.onicecandidate = (event) => {
          if (event.candidate) {
            sendSignal(senderId, {
              type: "candidate",
        ...
        setHistory((prev) => [
          {
            id: Date.now(),
            duration,
            splits: stateRef.current.splitTimes.map((s) => ({
              duration: s.duration,
    ...
    <div className="fixed inset-0 bg-black flex flex-col overflow-hidden touch-none text-white">
      ...
      <div className="absolute top-0 left-0 right-0 z-50 p-4 pt-[calc(1rem+env(safe-area-inset-top))] flex items-center justify-between pointer-events-none">
        <Button
          ...
          className="bg-black/60 backdrop-blur-md border-gray-600 text-white pointer-events-auto"
        >
          <ChevronLeft size={20} />
        </Button>
        ...
        {!isSetupMode && (
          <div
            className="bg-black/60 backdrop-blur-md px-3 py-1 rounded-full border border-gray-600 flex items-center gap-2 shadow-lg pointer-events-auto"
            ...
            {(splitTimes.length > 0 || gameState === "FINISHED") && (
              <div className="mt-6 w-full max-w-md space-y-2">
                ...
                {splitTimes.map((split, idx) => (
                  <div
                    key={idx}
                    className="flex items-center justify-between bg-blue-500/10 border border-blue-500/30 rounded-lg px-4 py-2"
                  >
              ...
              <div className="bg-gray-900 rounded-xl border border-gray-800 overflow-hidden">
                ...
                <div className="divide-y divide-gray-800">
                  ...
                  {(Object.values(peers) as WebRTCPeer[]).map((peer) => (
                    <div
                      key={peer.id}
                      className="p-4 flex items-center justify-between gap-3 bg-gray-900/50"
                    >
