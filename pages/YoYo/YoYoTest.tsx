...
class AudioEngine {
  ...
  public async loadStartSound(url: string) {
    if (!this.audioContext || this.startSoundBuffer || this.isLoadingStart)
      return;
    this.isLoadingStart = true;
    try {
      const response = await fetch(url);
      const arrayBuffer = await response.arrayBuffer();
      this.startSoundBuffer = await this.audioContext.decodeAudioData(
        arrayBuffer
      );
    } catch (e) {
  ...
  public async loadTurnSound(url: string) {
    if (!this.audioContext || this.turnSoundBuffer || this.isLoadingTurn)
      return;
    this.isLoadingTurn = true;
    try {
      const response = await fetch(url);
      const arrayBuffer = await response.arrayBuffer();
      this.turnSoundBuffer = await this.audioContext.decodeAudioData(
        arrayBuffer
      );
    } catch (e) {
  ...
  public playStartSound = () => {
    this.playBuffer(this.startSoundBuffer, () =>
      this.playTone(880, "square", 0.4)
    );
  };

  public playTurnSound = () => {
    this.playBuffer(this.turnSoundBuffer, () =>
...
const YoYoTest: React.FC = () => {
  ...
  const [warnedParticipants, setWarnedParticipants] = useState<Set<string>>(
    new Set()
  );
  ...
  useEffect(() => {
    ...
    const turnUrl =
      "https://video-idea.fra1.cdn.digitaloceanspaces.com/beeps/beep-short.mp3";
  ...
  const updateLoop = useCallback(() => {
    ...
    const phase = PROTOCOL.find(
      (p) =>
        totalElapsedSeconds >= p.startTime && totalElapsedSeconds < p.endTime
    );
    ...
    <div className="fixed inset-0 w-full h-full bg-gray-950 flex flex-col overflow-hidden">
      ...
      {activeTab === "test" ? (
        <>
          ...
          <div className="flex-1 flex flex-col min-h-0">
            ...
            <div className="flex-[2] flex flex-col items-center justify-center p-6 relative">
              ...
              <div
                className={`text-xl font-bold uppercase tracking-widest mb-2 transition-colors duration-300 ${getPhaseColor()}`}
              >
          ...
          <div className="flex-none bg-gray-900 border-t border-gray-800 p-6 pb-[env(safe-area-inset-bottom)]">
            <div className="max-w-md mx-auto flex items-center justify-between gap-6">
              <button
                onClick={resetTest}
                disabled={!hasStarted}
                className={`h-14 w-14 rounded-full flex items-center justify-center border border-gray-700 bg-gray-800 text-white transition-all ${
                  !hasStarted ? "opacity-30" : "hover:bg-gray-700"
                }`}
              ...
              <button
                onClick={
                  isRunning ? stopTest : hasStarted ? resumeTest : startTest
                }
        ...
        <div className="flex-1 flex flex-col min-h-0">
          ...
          <div
            className={`flex-none p-4 border-b border-gray-800 ${
              hasStarted
                ? currentPhase.type === "RUN"
                  ? "bg-emerald-900/30"
                  : "bg-orange-900/30"
                        ...
                        <>
                          <span
                            className={`text-lg font-bold tabular-nums ${
                              isWarned ? "text-yellow-300" : "text-white"
                            }`}
                          >
                            {distance}
                          ...
                          <button
                            ...
                            title={
                              isWarned ? "Mark as dropped out" : "Issue warning"
                            }
                          >
                            {isWarned ? (
                              <X size={16} />
              ...
              <span className="text-gray-400">
                ...
                {warnedParticipants.size > 0 && (
                  <span className="text-yellow-400 ml-1">
                    ({warnedParticipants.size} warned)
                  </span>
                )}
